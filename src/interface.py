...
class Interface:
    def __init__(self):
        ...
        caminho_fonte_personalizada = os.path.join(os.getcwd(), "res", "fonts", "fonte.ttf")
        
        try:
            self.fonte = pygame.font.Font(caminho_fonte_personalizada, 30)
            self.fonte_grande = pygame.font.Font(caminho_fonte_personalizada, 60)
        except FileNotFoundError:
            self.fonte = pygame.font.SysFont('Arial', 30, bold=True)
            self.fonte_grande = pygame.font.SysFont('Arial', 60, bold=True) 

        #chao
        caminho_chao = os.path.join(os.getcwd(), "res", "sprites_textura", "rua_universitaria.png")
        ...
        caminho_laje = os.path.join(os.getcwd(), "res", "sprites_textura", "chao_cin.png") 
        try:
    ...
    def mostrar_itens_coletados(self, tela, itens_coletados):
        ...
        for i in range(3):
            ...
            if i < len(itens_coletados):
                ...
                item_y = 20 + (self.moldura_itens.get_height() - frame.get_height()) // 2
    ...
    def desenhar_go(self, tela, liberado_para_avancar):
        if liberado_para_avancar:
            if (pygame.time.get_ticks() // 500) % 2 == 0: 
                ...
                tela.blit(texto_go, (config.largura - 245, config.altura // 7 + 5))
                texto_go = self.fonte_grande.render("GO >", True, (255, 255, 255)) 
