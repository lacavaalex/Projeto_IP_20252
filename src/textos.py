...
class Textos():
    ...
    def mostrar_tela_inicio(self):
        textos = [
            ...
            ("Pressione ENTER para JOGAR", self.centro_x, self.centro_y + 40),
            ("Pressione C para ver os controles", self.centro_x, self.centro_y + 80)
        ]
        self.interface.mostrar_tela(self.tela, "ROGÉRIO vs WALLYSON:", textos, (0, 0, 0))
    ...
    def mostrar_tela_controles(self):
        ...
        self.interface.mostrar_tela(self.tela, "CONTROLES", textos, (30, 30, 30))
    ...
    def mostrar_tela_checkpoint(self, item_coletado):
        ...
        textos = [
            ...
            ("Pressione ENTER", self.centro_x, self.centro_y + 40)
        ]
    ...
    def mostrar_tela_vitoria(self):  
        ...
        texto = [
            ("Você apresentou o projeto! Está APROVADO em IP!!", self.centro_x, self.centro_y),
            ("Parabéns pela vitória, e boa sorte no próximo período!", self.centro_x, self.centro_y + 40)
        ]
    ...
    def mostrar_tela_morte(self): 
        titulo = "GAME OVER"
        textos = [
            ("Você não conseguiu chegar a tempo,", self.centro_x, self.centro_y - 60),
            ("seu projeto não foi apresentado. Você REPROVOU.", self.centro_x, self.centro_y - 20),
            ('"Te vejo período que vem." - Recém-Monitor Wallyson', self.centro_x, self.centro_y + 20),
            ("Pressione ENTER para voltar ao menu", self.centro_x, self.centro_y + 80)
        ]
    ...
    def mostrar_tela_historia(self, fase_historia):
        ...
        if fase_historia == 1:
            ...
            textos = [
                ("É o dia da apresentação do projeto final de IP, Rogério.", centro_x, centro_y - 120),
                ...
                ('"Mizinga, Wallyson! Sempre me sabotando pra eu reprovar!"', centro_x, centro_y + 80),
                ...
                ("Mas antes, vá até o Churrasquito pra encher o bucho!", centro_x, centro_y + 240),
                ("Pressione ENTER para começar", centro_x, centro_y + 300)
            ]
            self.interface.mostrar_tela(self.tela, titulo, textos, (40, 40, 80)) 

        if fase_historia == 2:
            titulo = "O TRAIDOR APARECE!"
            
            textos = [
                ('"Rapaz, olha quem chegou cedo!", debochou Wallyson.', centro_x, centro_y - 100),
                ('"Por hora, só nós dois estamos aqui,"', centro_x, centro_y - 60),
                ('"então vamos acabar logo com isso."', centro_x, centro_y - 20),
                ('"Meus capangas de Guabiraba não são os melhores,"', centro_x, centro_y + 20),
                ('"Mas EU sou."', centro_x, centro_y + 60),
                ('"È hoje que tu volta a ser barman carpinense,"', centro_x, centro_y + 100),
                ...
                ("Pressione ENTER para continuar", centro_x, centro_y + 300)
            ]
            self.interface.mostrar_tela(self.tela, titulo, textos, (80, 40, 40))